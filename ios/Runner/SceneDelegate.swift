...
/// Captures a cold-start notification tap. When the app is killed, iOS delivers
/// the tap through the scene connection (not Firebase's swizzled path), so we
/// extract the destination URL and hand it to Dart via UserDefaults. The
/// `flutter.` prefix bridges UserDefaults ↔ SharedPreferences; the key must
/// match `ColdTapReader._key` on the Dart side.
class SceneDelegate: FlutterSceneDelegate {
  static let launchRouteKey = "flutter.ridge_launch_route"
  ...
  override func scene(
    ...
    #if DEBUG
    NSLog("[RDG.ROUTE] captured notification destination")
    #endif
  }
